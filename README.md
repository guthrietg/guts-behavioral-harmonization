...
Import the wave-specific raw survey components.

Standardize identifiers, names, timestamps, and other linkage fields.

Identify likely test or researcher-generated responses.

Resolve missing or shared participant identifiers where possible.

Resolve repeated informed-consent submissions and other identity collisions.

Review incomplete or ambiguous records through explicit audit files.

Link cleaned participant records to the stable project-level gutsID.

Perform additional quality-control checks before export.

Split cleaned questionnaires into measure-level datasets using the project measure registry/overview.

Export cleaned, consistently named measure-level outputs for downstream analysis.
...
For implementation details, the wave-specific R Markdown scripts are the authoritative source.ripts are the authoritative source.
